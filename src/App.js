...
import BookShelf from "./BookShelfView";
...
class BooksApp extends Component {
  ...
  fetchBooks = () => {
    BooksAPI.getAll().then(books => this.setState({ booksList: books }));
  };
  componentDidMount() {
    this.fetchBooks();
  }
  ...
  onChangeShelf = (bookToBeUpdated, newShelf, history) => {
    ...
    this.setState(prevState => {
      return {
        booksList: [
          ...prevState.booksList.filter(b => b.id !== bookToBeUpdated.id),
          bookToBeUpdated
        ]
      };
  ...
  getBookShelf = book => {
    let currentShelf = this.state.booksList
